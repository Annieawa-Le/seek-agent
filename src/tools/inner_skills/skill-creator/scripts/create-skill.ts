...
function genIndexTs(skillName: string, tools: { name: string; file: string }[], skillDirName: string): string {
  ...
  const imports = autoTools
    .map(t => `import { ${toolNameToVar(t.name)} } from './scripts/${t.file}';`)
    .join('\n');
...
export const createSkill = tool({
  ...
  execute: async ({ skillName, description, tools }): Promise<string> => {
    ...
    try {
      ...
      // 5. 创建 scripts/ 目录（存放工具实现文件）
      const scriptsDir = path.join(skillDir, 'scripts');
      await fs.mkdir(scriptsDir, { recursive: true });
      createdFiles.push(scriptsDir);

      // 6. 为每个工具创建实现文件到 scripts/
      ...
      // 7. 创建 scripts/prompt-get.ts（每个 skill 必备：获取技能说明文档）
      const promptGetFilePath = path.join(scriptsDir, 'prompt-get.ts');
      await fs.writeFile(promptGetFilePath, genPromptGetToolFile(skillDirName), 'utf-8');
      createdFiles.push(promptGetFilePath);

      // 8. 创建 index.ts（自动注入所有工具入口，含 prompt-get）
      ...
      return [
        ...
        '',
        '💡 下一步：',
        ...tools.map(t => `   - 编辑 scripts/${toolNameToFile(t.name)}.ts 实现 "${t.name}" 的具体逻辑`),
